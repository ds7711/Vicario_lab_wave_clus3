...
par.sr = 25000;                      % sampling rate (in Hz). This parameter will be only used if the data file don't have a sr. 
...
par.max_spikes_plot = 10;          % max. number of spikes to be plotted 
 
par.print2file = true;               % If is not true, print the figure (only for batch scripts). 
 
par.cont_plot_samples = 100;      % number of samples used in the one-minute (maximum) sample of continuous data to plot. 
...
par.w_post = 22;                     % number of post-event data points stored (default 44)) 
